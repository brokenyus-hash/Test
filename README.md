...
- **Typed commands** for when you want something to happen, full stop. Start a message with `/`: `/force She kisses you` (hard direction, whoever speaks next makes it happen), `/narrate A stranger walks in`, `/say Jessica: "Fine. Get in."` (you write her line yourself; no AI involved), `/steer keep it tense`, `/time the next morning`, `/scene the harbour, two days later`, `/twist`, `/enter Marcus: her older brother, a cop` (joins the cast right now), `/leave Marcus`, `/note keep it noir` (standing direction), `/ooc …`. Type `/help` for the list. Stage directions are marked authoritative in the prompt: characters may react with reluctance, but the directed event always happens.
